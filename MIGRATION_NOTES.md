...
3. **Updated `pyproject.toml`**: Minimal configuration
   - Contains basic uv and Sphinx configuration
   - No [project] section (this is a documentation site, not a package)
...
1. **`.github/workflows/deploy.yml`**: Updated CI/CD workflow
   ...
   - Added `uv venv` to create virtual environment
   - Changed `pip install` to `uv pip install -r requirements.txt`
   - Activated venv before running Sphinx
...
2. **Simple Workflow**: `uv venv` + `uv pip install` is straightforward
3. **Better DX**: Fast virtual environment creation and package resolution
4. **Python Version Management**: Automatic Python version handling with `.python-version`
5. **Backward Compatible**: Works seamlessly with existing requirements.txt
...
**Add dependency:**
```bash
# Edit requirements.txt to add dependency
uv pip install -r requirements.txt
```
...
- **uv users**: Use `uv venv` + `uv pip install -r requirements.txt`
- **pip users**: Can still use `pip install -r requirements.txt` (no changes!)
