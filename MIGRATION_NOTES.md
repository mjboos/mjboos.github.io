...
1. **`pyproject.toml`**: Modern Python project configuration
   - Contains [project] section with metadata and dependencies
   - **Deliberately omits [build-system] section** - this makes uv treat it as a "virtual project"
   - When [build-system] is absent, uv installs dependencies but doesn't build/install the project
   - This is the official uv pattern for documentation-only projects
...
3. **`uv.lock`**: Dependency lockfile
   - Generated by `uv lock` (automatically created during `uv sync`)
...
1. **`.github/workflows/deploy.yml`**: Updated CI/CD workflow
   ...
   - Changed to `uv sync` for dependency installation
   - Uses `uv run sphinx-build` for building
   - Enabled uv caching for faster builds

2. **`noxfile.py`**: Updated build automation
   - Added `venv_backend="uv"` to nox session
   - Uses `session.install(".")` which syncs from pyproject.toml
...
1. **Faster Installation**: Package installation with uv is 10-100x faster than pip
2. **Reproducible Builds**: `uv.lock` ensures exact same versions everywhere
3. **Simple Workflow**: Single `uv sync` command installs everything
4. **Pure uv-native**: Uses standard `uv sync` and `uv run` commands
5. **Python Version Management**: Automatic Python version handling with `.python-version`
6. **Virtual Project Pattern**: Official uv approach for non-package projects (no [build-system])
...
**Add dependency:**
```bash
# Edit pyproject.toml [project] dependencies section
...
- **pip users**: Can still use `pip install -r requirements.txt`
- **CI/CD**: Fully migrated to uv with `uv sync` (faster builds!)
- **nox**: Uses uv backend for faster environment creation
- **Dependencies**: pyproject.toml is source of truth, requirements.txt kept for compatibility

## Rollback

If needed, revert to pip by:
1. Update `.github/workflows/deploy.yml` to use `actions/setup-python` and `pip install`
2. Update `noxfile.py` to remove `venv_backend="uv"`
3. Continue using `requirements.txt` (no changes needed!)
4. `.python-version` and `pyproject.toml` can remain (harmless)
